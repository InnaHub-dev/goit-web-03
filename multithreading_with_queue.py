...
def get_folders(
    directory: Path, sorting_dictionary: dict, end_condition: Event, q: Queue
):
...
def sort_and_move_files(
    ...
    """
    while True:
        if q.empty() and end_condition.is_set:
            return "All sorted!"
        else:
            folder = q.get()
            if not folder:
...
def main():
    ...
    get_folders(directory, SORTING_DICT, end_condition, q)
    threads = []
    for number in range(3):
        th = Thread(
            target=sort_and_move_files,
            args=(directory, q, SORTING_DICT, sorted_dict, end_condition),
        )
        th.start()
        threads.append(th)
